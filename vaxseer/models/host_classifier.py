...
# ============================================================================
# APPEND THIS TO: vaxseer/models/host_classifier.py
# A bi-encoder HI regressor using a pretrained ESM-2 (single-sequence) backbone.
# Parallel to ESMRegressor (which uses the MSA Transformer); leaves it untouched.
#
# Uses the REAL fair-esm (via models/real_esm2_loader.py) because the repo's
# vendored esm/ is an old fork that cannot build ESM-2 (no rotary embeddings).
# Install once:  pip install fair-esm==2.0.0
# ============================================================================
from models.real_esm2_loader import load_esm2


@register_model("esm2_regressor")
class ESM2Regressor(LightningModule):
    def __init__(self, config, alphabet=None, **args) -> None:
        super().__init__()
        self.save_hyperparameters(ignore=["alphabet"])
        self.config = config

        # load pretrained ESM-2 via the real fair-esm (own alphabet + converter)
        self.esm2, self.esm2_alphabet = load_esm2(getattr(config, "esm2_model", "esm2_t30_150M_UR50D"))
        self.batch_converter = self.esm2_alphabet.get_batch_converter()
        self.repr_layer = self.esm2.num_layers
        embed_dim = self.esm2.embed_dim

        # optionally freeze the backbone (default: frozen, train head only)
        self.freeze_esm2 = getattr(config, "freeze_esm2", True)
        if self.freeze_esm2:
            for p in self.esm2.parameters():
                p.requires_grad = False

        # alphabet kept for compatibility with bin/train.py data module setup
        self.alphabet = alphabet

        self.criterion = torch.nn.MSELoss(reduce=False)
        self.classifier_heads = self.build_prediction_heads(config, embed_dim)

    def build_prediction_heads(self, config, embed_dim):
        heads = nn.ParameterDict()
        # pair representation is [v, r, v-r, v*r] -> 4 * embed_dim
        in_dim = embed_dim * 4
        for task in config.labels:
    ...
    @classmethod
    def add_argparse_args(cls, parent_parser):
        parent_parser.add_argument('--esm2_model', type=str, default="esm2_t30_150M_UR50D")
        parent_parser.add_argument('--freeze_esm2', type=str2bool, default="true")
        parent_parser.add_argument('--dropout', type=float, default=0.1)
        parent_parser.add_argument('--adam_epsilon', type=float, default=1e-8)
        parent_parser.add_argument('--load_from_checkpoint', type=str, default=None)
        parent_parser.add_argument('--predict_numerical_output', type=str, default="average", choices=["argmax", "average"])
    ...
    def overwrite_generate_kwargs(self, args):
        setattr(self.config, "predict_index_path", args.predict_index_path)
        setattr(self.config, "predict_numerical_output", getattr(args, "predict_numerical_output", "average"))

    def _encode(self, list_of_seq_lists):
        """list_of_seq_lists: list (len B) of lists of raw sequences for one side.
        Returns [B, embed_dim] using the BOS representation, averaged over rows."""
        reps = []
        for seqs in list_of_seq_lists:
            data = [("s%d" % i, s) for i, s in enumerate(seqs)]
            _, _, toks = self.batch_converter(data)
            toks = toks.to(self.device)
            with torch.set_grad_enabled(not self.freeze_esm2):
                out = self.esm2(toks, repr_layers=[self.repr_layer], return_contacts=False)
            h = out["representations"][self.repr_layer][:, 0]  # BOS token, [rows, D]
            reps.append(h.mean(0))  # average over rows (1 row in plain HI)
        return torch.stack(reps, dim=0)  # [B, D]

    def forward(self, batch, batch_idx, mode="train"):
        v = self._encode(batch["virus_seqs"])    # [B, D]
        r = self._encode(batch["vaccine_seqs"])  # [B, D]
        pair = torch.cat([v, r, v - r, v * r], dim=-1)  # [B, 4D]

        if mode == "test":
            out = {}
            for key in self.classifier_heads:
                out[key] = self.classifier_heads[key](pair).squeeze(-1)
            return out
        else:
            loss_dict = {}
            loss_all = []
            for key in self.classifier_heads:
                logit = self.classifier_heads[key](pair).squeeze(-1)
                loss = self.criterion(logit, batch[key].float())
                loss_dict[key] = torch.mean(batch["loss_weight"] * loss)
                loss_all.append(loss_dict[key])
                loss_dict[key + "_no_reweight"] = torch.mean(loss)
            return sum(loss_all), loss_dict

    def training_step(self, batch, batch_idx):
        loss, loss_dict = self.forward(batch, batch_idx, mode="train")
        self.log_dict({"train_" + k: v for k, v in loss_dict.items()})
    ...
    # --- testing / prediction mirror ESMRegressor's interface ---
    def testing_forward(self, batch):
        logits = self.forward(batch, None, mode="test")
        prediction_dict = {}
        for key in self.classifier_heads:
    ...
    def test_step(self, batch, batch_idx):
        return self.testing_forward(batch)
    ...
    def output_testing_results(self, outputs, test_datasets):
        ...
        output_set = []
        for i, item in enumerate(test_dataset):
            new_item = {"src_id1": item["src_id1"], "src_id2": item["src_id2"]}
            for key in outputs:
                new_item["pred_" + key] = outputs[key][i].item()
                new_item[key] = item[key]
            output_set.append(new_item)
