...
def train(args, model, dm):
    ...
    # args.early_step
    callbacks = [
            ModelCheckpoint(save_weights_only=False, mode=args.model_ckpt_mode, monitor=args.model_ckpt_monitor, save_last=True, save_top_k=-1, every_n_epochs=1),
    ...
    import glob
    last_ckpts = glob.glob(os.path.join(root_dir, "**", "last.ckpt"), recursive=True)
    if last_ckpts and not (args.test or args.predict):
        resume_ckpt = max(last_ckpts, key=os.path.getmtime)
        logging.info("Auto-resuming from checkpoint: %s" % resume_ckpt)
        args.resume_from_checkpoint = resume_ckpt
