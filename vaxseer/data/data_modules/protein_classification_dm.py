...
class PairwiseRegressionAlnDataModule(PairwiseRegressionDataModule):
    ...
    def build_predict_datasets(self, properties, properties_dict, *args, **argv):
        ...
        pred_dataset = PairwiseAlnClassificationDataset(pred_data, self.vocab, category=self.args.category,
            ...
            numerical_interval=self.args.numerical_interval, numerical=self.args.numerical)
# ============================================================================
# APPEND THIS TO: vaxseer/data/data_modules/protein_classification_dm.py
# Reuses all dataset-building logic of hi_regression_aln, but overrides
# set_collate_func so the ESM-2 pair collate (raw sequences) is used instead
# of the MSA collate. set_collate_func is the single method base_dm.setup()
# calls to install the collate, and the dataloaders read self.collate_fn at
# call time, so overriding here makes our collate win for train/val/test/predict.
#
# Requires at top of this file:  from data.utils import esm2_pair_collate_func
# ============================================================================
from functools import partial as _esm2_partial
from data.utils import esm2_pair_collate_func


@register_dm("hi_regression_esm2")
class PairwiseRegressionESM2DataModule(PairwiseRegressionAlnDataModule):
    def __init__(self, args, vocab=None):
        super().__init__(args, vocab)

    def set_collate_func(self, properties_dict, *args, **kwargs):
        # install the ESM-2 pair collate instead of the MSA collate.
        # batch_converter is unused (ESM-2 tokenizes inside the model).
        self.collate_fn = _esm2_partial(
            esm2_pair_collate_func,
