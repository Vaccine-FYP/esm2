...
def remove_gaps_from_source(src_tokens, tgt_tokens, gap_idx, pad_idx): # self.alphabet.gap_idx
    ...
    #     for atom in new_c2:
# ============================================================================
# APPEND THIS TO: vaxseer/data/utils.py
# Collate for ESM-2 bi-encoder HI regression. Keeps raw per-side sequences
# (gaps stripped) instead of MSA-tokenizing; ESM-2 tokenizes inside forward.
# Self-contained: explicitly tensorizes numeric keys (label, loss_weight) and
# passes ids/index through as lists, so it does not depend on collate_properties.
# ============================================================================
import torch as _esm2_torch


def esm2_pair_collate_func(list_of_dict, batch_converter=None, padding_idx=None, properties_dict={}):
    virus_seqs = []   # list (len B) of lists of raw virus sequences
    vaccine_seqs = []  # list (len B) of lists of raw vaccine sequences
    for x in list_of_dict:
        seqs = x["src_seq"]      # seqs1 + seqs2 (virus rows then vaccine rows)
        labels = x["seq_label"]  # 0 for virus rows, 1 for vaccine rows
        v = [s.replace("-", "") for s, l in zip(seqs, labels) if l == 0]
        r = [s.replace("-", "") for s, l in zip(seqs, labels) if l == 1]
        virus_seqs.append(v if v else [""])
        vaccine_seqs.append(r if r else [""])

    ret = {"virus_seqs": virus_seqs, "vaccine_seqs": vaccine_seqs}

    sample = list_of_dict[0]
    skip = {"src_seq", "seq_label", "ref_seq_label", "src_time"}
    for key in sample.keys():
        if key in skip:
            continue
        vals = [x[key] for x in list_of_dict]
        v0 = vals[0]
        # numeric -> float/long tensor; everything else (ids, strings) -> list
        if isinstance(v0, bool):
            ret[key] = _esm2_torch.tensor(vals)
        elif isinstance(v0, int):
            ret[key] = _esm2_torch.tensor(vals, dtype=_esm2_torch.long)
        elif isinstance(v0, float):
            ret[key] = _esm2_torch.tensor(vals, dtype=_esm2_torch.float)
        else:
            try:
                # numpy scalars etc.
                ret[key] = _esm2_torch.tensor([float(x) for x in vals], dtype=_esm2_torch.float)
            except (ValueError, TypeError):
                ret[key] = vals  # ids / strings stay as a list
    return ret
