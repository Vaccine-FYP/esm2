...
def read_fasta(path, max_date = "9999-12-31"):
    ...
    for record in SeqIO.parse(path, "fasta"):
        ...
        try:
            ...
            id2records[descs[3].replace(" ", "_").replace("-", "_").lower()].add(str(record.seq))
            id2accids[descs[3].replace(" ", "_").replace("-", "_").lower()].add(ha_id)
            
            accid2id[ha_id] = descs[3].replace(" ", "_").replace("-", "_").lower()
